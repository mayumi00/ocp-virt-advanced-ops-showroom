...
NOTE: 本コースでは、Webコンソールのメニューが英語表記となっています。メニューを英語表記にするために次のことを実施してください。Webコンソール画面右上のユーザ名をクリックして [ユーザー設定] のメニューを開きます。次に表示される画面で [言語] メニューを選択すると、「Use the default browser language setting.」のチェックボックスにチェックが入っています。このチェックボックスのチェックを外し、[言語の設定] のドロップダウンリストから、[English] を選択します。最後にブラウザで再読み込みすると、英語のWebコンソールが表示されます。
